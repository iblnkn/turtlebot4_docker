...
ENV RMW_IMPLEMENTATION=rmw_${RMW_MIDDLEWARE}_cpp
...
RUN apt-get update \
    && apt-get install -y "ros-${ROS_DISTRO}-ros-gz${GZ_VERSION#fortress}" emacs htop byobu python3 pip less ros-${ROS_DISTRO}-teleop-twist-keyboard ros-${ROS_DISTRO}-desktop ros-${ROS_DISTRO}-rmw-zenoh-cpp\
    && rm -rf /var/lib/apt/lists/*
